...
namespace detail {
   using eosio::detail::max_stack_buffer_size;
   ...
   class table_base {
    protected:
      ...
      void put(const void* value, void* old_value,
         ...
         auto primary_key_found = ::eosio::internal_use_do_not_use::kv_get(contract_name.value, tbl_key.data(), tbl_key.size(), value_size);
         ...
         if (primary_key_found) {
            ...
            auto copy_size = ::eosio::internal_use_do_not_use::kv_get_data(0, buffer.get(), value_size);
         ...
         for (const auto& idx : secondary_indices) {
            ...
            auto sec_found = ::eosio::internal_use_do_not_use::kv_get(contract_name.value, sec_tbl_key.data(), sec_tbl_key.size(), value_size);

            if (!primary_key_found) {
               eosio::check(!sec_found, "Attempted to store an existing secondary index.");
               ::eosio::internal_use_do_not_use::kv_set(contract_name.value, sec_tbl_key.data(), sec_tbl_key.size(), tbl_key.data(), tbl_key.size(), payer.value);
            } else {
               if (sec_found) {
                  auto free_memory = [value_size](char* buf) { if (detail::max_stack_buffer_size < value_size) free(buf);};
                  std::unique_ptr<char, decltype(free_memory)> buffer( (char*)(detail::max_stack_buffer_size < value_size ? malloc(value_size) : alloca(value_size)), free_memory);
                  ...
                  ::eosio::internal_use_do_not_use::kv_erase(contract_name.value, old_sec_key.data(), old_sec_key.size());
                  ::eosio::internal_use_do_not_use::kv_set(contract_name.value, sec_tbl_key.data(), sec_tbl_key.size(), tbl_key.data(), tbl_key.size(), payer.value);
               }
            }
         }
         ...
         ::eosio::internal_use_do_not_use::kv_set(contract_name.value, tbl_key.data(), tbl_key.size(), data_buffer.get(), data_size, payer.value);
      }
      ...
      void erase(const void* value) {
         ...
         for (const auto& idx : secondary_indices) {
            auto sec_tbl_key = full_key(make_prefix(table_name, idx->index_name), idx->get_key_void(value));
            ::eosio::internal_use_do_not_use::kv_erase(contract_name.value, sec_tbl_key.data(), sec_tbl_key.size());
         }

         ::eosio::internal_use_do_not_use::kv_erase(contract_name.value, tbl_key.data(), tbl_key.size());
      }
   };
   ...
   inline void kv_index::get(const full_key& key, void* ret_val, void (*deserialize)(void*, const void*, std::size_t)) const {
      ...
      auto success = ::eosio::internal_use_do_not_use::kv_get(contract_name.value, key.data(), key.size(), value_size);
      if (!success) {
         return;
      }
      ...
      if (!is_primary) {
         auto success = ::eosio::internal_use_do_not_use::kv_get(contract_name.value, buffer.get(), copy_size, actual_data_size);
         ...
         auto pk_copy_size = ::eosio::internal_use_do_not_use::kv_get_data(0, pk_buffer.get(), actual_data_size);
   ...
   public:
      ...
      ~iterator_base() {
         if (itr) {
            ::eosio::internal_use_do_not_use::kv_it_destroy(itr);
         }
      }
     ...
     void value(void* val, void (*deserialize)(void*, const void*, std::size_t)) const {
         ...
         ::eosio::internal_use_do_not_use::kv_it_value(itr, 0, (char*)nullptr, 0, value_size);

         auto free_memory = [value_size](char* buf) { if (detail::max_stack_buffer_size < value_size) free(buf);};
         std::unique_ptr<char, decltype(free_memory)> buffer( (char*)(detail::max_stack_buffer_size < value_size ? malloc(value_size) : alloca(value_size)), free_memory);

         auto stat = ::eosio::internal_use_do_not_use::kv_it_value(itr, offset, buffer.get(), value_size, actual_value_size);
         ...
         if (!is_primary) {
            auto success = ::eosio::internal_use_do_not_use::kv_get(index->contract_name.value, buffer.get(), actual_value_size, actual_data_size);
            eosio::check(success, "failure getting primary key in `value()`");

            auto free_memory = [actual_data_size](char* buf) { if (detail::max_stack_buffer_size < actual_data_size) free(buf);};
            std::unique_ptr<char, decltype(free_memory)> pk_buffer( (char*)(detail::max_stack_buffer_size < actual_data_size ? malloc(actual_data_size) : alloca(actual_data_size)), free_memory);

            ::eosio::internal_use_do_not_use::kv_get_data(0, pk_buffer.get(), actual_data_size);
      ...
      full_key key() const {
         ...
         ::eosio::internal_use_do_not_use::kv_it_key(itr, 0, (char*)nullptr, 0, value_size);

         auto free_memory = [value_size](char* buf) { if (detail::max_stack_buffer_size < value_size) free(buf);};
         std::unique_ptr<char, decltype(free_memory)> buffer( (char*)(detail::max_stack_buffer_size < value_size ? malloc(value_size) : alloca(value_size)), free_memory);

         auto stat = ::eosio::internal_use_do_not_use::kv_it_key(itr, 0, buffer.get(), value_size, actual_value_size);
   ...
   protected:
      ...
      int compare(const iterator_base& b) const {
         ...
         if (a_is_end && b_is_end) {
            ...
            return ::eosio::internal_use_do_not_use::kv_it_compare(itr, b.itr);
         }
      }
   };
 ...
 * @tparam T         - the type of the data stored as the value of the table
  */
template<typename T, eosio::name::raw TableName>
class table : detail::table_base {

   using kv_index = detail::kv_index;

   class base_iterator : public detail::iterator_base {
   public:
   ...
   public:
      ...
      iterator& operator++() {
         ...
         itr_stat = static_cast<status>(::eosio::internal_use_do_not_use::kv_it_next(itr));
         return *this;
      }

      iterator& operator--() {
         if (!itr) {
            itr = ::eosio::internal_use_do_not_use::kv_it_create(index->contract_name.value, index->prefix.data(), index->prefix.size());
         }
         itr_stat = static_cast<status>(::eosio::internal_use_do_not_use::kv_it_prev(itr));
      ...
      int32_t key_compare(const full_key& kt) const {
         if (itr == 0 || itr_stat == status::iterator_end) {
            ...
            return ::eosio::internal_use_do_not_use::kv_it_key_compare(itr, kt.data(), kt.size());
         }
      }
   ...
   public:
      ...
      reverse_iterator& operator--() {
         if (!itr) {
            itr = ::eosio::internal_use_do_not_use::kv_it_create(index->contract_name.value, index->prefix.data(), index->prefix.size());
            itr_stat = static_cast<status>(::eosio::internal_use_do_not_use::kv_it_lower_bound(itr, "", 0));
         }
         itr_stat = static_cast<status>(::eosio::internal_use_do_not_use::kv_it_next(itr));
         eosio::check(itr_stat != status::iterator_end, "decremented past the beginning");
         return *this;
...
public:
   ...
   public:
      using iterator = table::iterator;
      using table<T,TableName>::kv_index::tbl;
      using table<T,TableName>::kv_index::table_name;
      using table<T,TableName>::kv_index::contract_name;
      using table<T,TableName>::kv_index::index_name;
      using table<T,TableName>::kv_index::prefix;
      ...
      iterator find(const full_key& key) const {
         uint32_t itr = ::eosio::internal_use_do_not_use::kv_it_create(contract_name.value, prefix.data(), prefix.size());
         int32_t itr_stat = ::eosio::internal_use_do_not_use::kv_it_lower_bound(itr, key.data(), key.size());

         auto cmp = ::eosio::internal_use_do_not_use::kv_it_key_compare(itr, key.data(), key.size());
      ...
      bool exists(const full_key& key) const {
         uint32_t value_size;
         return ::eosio::internal_use_do_not_use::kv_get(contract_name.value, key.data(), key.size(), value_size);
      }
      ...
      iterator begin() const {
         uint32_t itr = ::eosio::internal_use_do_not_use::kv_it_create(contract_name.value, prefix.data(), prefix.size());
         int32_t itr_stat = ::eosio::internal_use_do_not_use::kv_it_lower_bound(itr, "", 0);
      ...
      reverse_iterator rbegin() const {
         uint32_t itr = ::eosio::internal_use_do_not_use::kv_it_create(contract_name.value, prefix.data(), prefix.size());
         int32_t itr_stat = ::eosio::internal_use_do_not_use::kv_it_prev(itr);
   ...
   void put(const T& value) {
      T old_value;
      table_base::put(&value, &old_value, &get_size_fun, &deserialize_fun, &serialize_fun);
   }
   ...
   static void deserialize_optional_fun(void* value, const void* buffer, std::size_t buffer_size) {
      ...
      return table::deserialize(**static_cast<std::optional<T>*>(value), buffer, buffer_size);
   }
   static void deserialize_fun(void* value, const void* buffer, std::size_t buffer_size) {
      return table::deserialize(*static_cast<T*>(value), buffer, buffer_size);
   }
   static void serialize_fun(const void* value, void* buffer, std::size_t buffer_size) {
      return table::serialize(*static_cast<const T*>(value), buffer, buffer_size);
   }
   static std::size_t get_size_fun(const void* value) {
      return table::get_size(*static_cast<const T*>(value));
   }
