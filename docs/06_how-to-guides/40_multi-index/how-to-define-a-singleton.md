...
For ease of use, define a type alias `singleton_type` based on the `eosio::singleton` template type, parametarized with a random name `"testtable"` and the `testtable` data structure. The names must adhere to EOSIO-Taurus account name restrictions.
...
| A full example project demonstrating the instantiation and usage of singleton can be found [here](../../../examples/singleton_example).
...
* Singleton uses as underlying structure a multi-index table therefore you can [iterate and retrieve data](./how-to-iterate-and-retrieve-a-multi_index-table.md) from the singleton the same way you would with a multi-index table.
